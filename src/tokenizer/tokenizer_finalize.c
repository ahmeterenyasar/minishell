...
/*   Updated: 2025/07/07 11:55:56 by ayasar           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

int	finalize_concatenated_word(char **word_parts, int part_count, int start_pos,
		int expandable, t_token **head, const char *input, int i)
{
	...
	if (part_count == 1 && is_quote_char(input[start_pos]))
		return (handle_single_quoted_token(input, start_pos, word_parts,
				head) ? i : i);
